...
| **C6** | **How is a detected feature weighted for relevance?** *(new, from E02)* | open — **critical path** | agent | every positional agent, all of Tier 2, [[evaluation]] | Three candidate routes, all needing machinery already planned: co-occurrence with the player's evaluation losses (E01 layer); recurrence across games (C2 rule); comparison against a rating-peer population (Lichess open database, free). Pick and test one → E03. The peer-population route is doubly attractive: the same corpus serves as evaluation control D2. |
