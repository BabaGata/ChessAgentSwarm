...
| **Embrey, D., *Understanding Human Behaviour and Error*, Human Reliability Associates** — [free PDF](https://mail.humanreliability.com/articles/Understanding%20Human%20Behaviour%20and%20Error.pdf), held in `docs/pdf/` | **the source of the four-way gap taxonomy** → [[domain.expertise-research]] § 6. Rasmussen's Skill-Rule-Knowledge classification and Norman/Reason's slips-vs-mistakes distinction, which the project's `GapTypeHypothesis` is a close relative of | **expert-consensus / standard framework** | A practitioner summary rather than the primary papers (Rasmussen 1979–1987, Norman 1981, Reason 1990), which are not freely available; it cites them throughout and is quotable. Two things it settles: the project's `SKILL` and `PROCESS` names are **inverted** against the standard framework, and `PSYCHOLOGICAL` is **not an error type** there but a performance-influencing factor |
