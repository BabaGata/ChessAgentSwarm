...
| E31 | does the swarm *see* what a reviewer sees, separately from whether it *says* it? | done — **detection 80 %, naming 10 %**. The analysis core lands on the same move 16 times in 20; it has a matching name twice. **All 9 pawn notes unnameable**, and the reviewer works in the 4–10 wp band below `INACCURACY_WP` | [[experiments.e31-move-level-agreement]] |
