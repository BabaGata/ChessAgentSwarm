...
| R-15 | **Regression to the mean makes any coaching system look effective** — a weakness is diagnosed because its rate was extreme, and re-measuring an extreme value returns a lower one whether or not anything was done | **critical** | **measured in E05: 92 % of targets met with no intervention** | Targets must be set against a shrunk estimate of the player's true rate, or against a measured control, never against the selected value that produced the finding. [[experiments.e05-natural-drift]] is re-run after any change to the target rule. Until then the progress check's verdicts are **not evidence that coaching works** and are not presented as such |
