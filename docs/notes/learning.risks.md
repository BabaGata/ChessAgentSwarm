...
| R-14 | **Correct but irrelevant output** — the swarm states things that are true, specific, evidence-backed and worthless ("you have an isolated pawn", true in 96 % of games). No correctness test catches this, because nothing is wrong | high | **demonstrated in E02** | Every feature-presence claim must be paired with relevance evidence (co-occurrence with loss, recurrence, or peer comparison). [[evaluation]] must test for usefulness separately from correctness — see L-007 |
