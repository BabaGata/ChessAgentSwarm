...
0. **P0 — Opening knowledge as development behaviour** *(2026-08-29)* →
   ...
   **Expectation now built from strong players** ([[experiments.e59-strong-player-expectation]]):
   `late_castling` and `repeat_move_in_opening` proceed, `pawn_moves_in_opening` is held because it
   discriminates in the **opposite** direction to the hypothesis, and `slow_development` still needs
   the same separation test. The remaining blocker is the **peer** half — separating 1600 from 2600
