...
| Area | Exists | Notes |
|---|---|---|
| Documentation / steering | yes | full vault; [[open-questions]] register; 21 ADRs |
...
| Measured evidence | **87 experiments** | [[experiments]] — `e01`–`e89`, the negative results included |
...
| Working detectors | **superseded** | the four experiment detectors (outpost, isolated pawn, backward pawn, rook on open file) now live in S5 pawn structure and S6 squares and files |
...
| Architecture **built** | **built** | `chesscoach/` — ingest, analysis core, profile, CLI, and since then the sections, arbiter, planner, explainer and prober |
...
| **S1 tactical gaps** | **built, assessed** | eight motif detectors at first (nine today), precision-gated by E04; took the swarm from 1 claim kind to **6** → [[mission.step-07-second-iteration]] |
...
| **The thesis** | **submission draft 2026-09-15** | [[thesis]] — in `Masters-thesis/` (gitignored), built to `AgataVujic_Diplomski_rad.docx` by `tools/build_docx.py`. Every number is a macro in `Src/brojke.tex`, one file, each with its source note; the scale figures were refreshed on 2026-09-15. **No longer a mission step** ([[decisions.0020-the-thesis-leaves-the-mission]]). Its first pass is recorded below, because it found things |
