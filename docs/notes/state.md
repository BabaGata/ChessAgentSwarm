...
0. **P0 — Opening knowledge as development behaviour** *(2026-08-29)* →
   ...
   **Expectation now built from strong players** ([[experiments.e59-strong-player-expectation]]):
   after standardising for the opening mix, **`late_castling` proceeds at full strength**,
   **`repeat_move_in_opening` at half**, and **`pawn_moves_in_opening` is dropped** — no
   within-opening effect on the measure that matches the hypothesis. `slow_development` still needs
   the same test. The remaining blocker is the **peer** half — separating 1600 from 2600
