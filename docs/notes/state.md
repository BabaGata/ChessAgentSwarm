...
0. **P0 — Opening knowledge as development behaviour** *(2026-08-29)* →
   ...
   **`repeat_move_in_opening` at half strength**, and **`pawn_moves_in_opening` is dropped**.
   ...
   blocker has moved to the **arbiter** — a claim with no cost cannot win a plan slot against
   claims that have one. **That is a decision for the author**, not a measurement: price the
   claims, let cost break ties instead of dominating, or reserve a slot for a habit claim. The remaining blocker is the **peer** half — separating 1600 from 2600
