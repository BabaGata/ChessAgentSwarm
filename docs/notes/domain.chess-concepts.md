...
| "Study endgames first" (Capablanca) vs "tactics first" | contested — but the Capablanca side is now sourced **primarily**: *Chess Fundamentals* Chapter I is literally "First Principles: Endings, Middle-game and Openings", in that order, opening with simple mates to teach the power of the pieces ([[domain.sources]]) | Sequencing decision; still testable per player via phase-wise error data. Note both traditions agree on what comes *last*: openings |
