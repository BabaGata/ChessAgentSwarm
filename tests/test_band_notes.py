...
class TestTheEstimateNamesItsSpeed:
    ...
    def test_a_blitz_estimate_admits_it_is_the_weaker_reading(self):
        # Measured on strangers: out by ~130 points after E29's attenuation
        # correction (was 150 before it), and still biased low. A player
        # anchoring on the number is owed both facts.
        from chesscoach.explainer import render

        report = render(self.profile_with("blitz", typical_error=129))
